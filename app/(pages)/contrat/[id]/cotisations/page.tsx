...
export default function CotisationPage() {
  ...
  const filteredAndSortedCotisations = useMemo(() => {
    if (!cotisations) return []; // Handle null/undefined cotisations

    const lowerCaseQuery = searchQuery.toLowerCase();
    const filtered = cotisations.filter((cotisation) =>
      String(cotisation.NumeroQuittance).toLowerCase().includes(lowerCaseQuery)
    );
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
        {/* Header Section */}
        <motion.div
          className="bg-gradient-to-r from-[#223268] to-[#1a254d] text-white rounded-lg p-6 mb-6 shadow-lg"
          ...
          <Grid container spacing={3} alignItems="flex-end">
            <Grid>
              <DatePicker
                ...
                className="w-full" // Tailwind for full width
              />
            </Grid>
            <Grid>
              <DatePicker
        ...
        >
          <CardContent className="p-0">
            <Typography
              variant="h6"
              className="px-6 pt-6 pb-2 text-gray-800 font-semibold"
            >
                        ...
                        <TableCell className="text-center text-sm px-4 py-3">
                          {cotisation.EtatQuittance === "Impayée" && (
                            <Button
                              variant="outlined"
                              color="primary"
                              className="!bg-[#223268] !text-white !normal-case !text-xs !py-1 !px-3 hover:!bg-[#1a254d] transition-colors"
                            >
                              Régler
                            </Button>
                          )}
